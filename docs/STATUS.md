...
### 레벨 레이아웃 (바퀴 #17 재설계) — 모든 플랫폼 원웨이(PlatformEffector2D)
- 지면 top = -3.25. 점프 apex(feet) ≈ 2.79u (jumpForce 13 / gravityScale 3 / mass 1).
- 좌측 타워(우상향 계단, 폭 6/4/3): L1 (-8,-1.50) / L2 (-6,0.50) / L3 (-4.5,2.50). top -1.35 / 0.65 / 2.65.
- 우측 타워(지그재그, 폭 5/3.5/2.5): R1 (7,-1.90) / R2 (8,0.15) / R3 (6.5,2.15). top -1.75 / 0.30 / 2.30.
- 층간 top-to-top 간격 1.9~2.05u. 각 상단 플랫폼 x 범위가 하단과 1.5u+ 겹쳐 직상 점프로 안착.
- 카메라 Y 는 0 고정(CameraFollow) → 화면 세로 -5..5. L3 위 플레이어 머리 ≈ 4.45 로 상단 아슬하게 보임.
  더 높은 층이 필요하면 CameraFollow 에 Y 추적/확대를 넣어야 함.
...
- (해결·바퀴 #17) 레벨을 다층으로 재설계. "아래 발판이 위 발판 점프를 막는" 문제는
...
- 한 일: INBOX(05:26) 슬라이스 A — 레벨 재설계(INBOX 항목 1 + 3). Game.unity 만 수정.
  · 일반(양면) `AirPlatform` 제거 + `OneWayPlatform_A/B` 제거 → 6개 원웨이 플랫폼으로 교체.
    전부 `PlatformEffector2D`(useOneWay, surfaceArc 180) + `BoxCollider2D.usedByEffector` +
    OneWayPlatform 레이어 + 시안 스프라이트.
  · 점프 도달 높이 먼저 계산: jumpForce 13 Impulse / mass 1 → v0 13, 유효중력 29.43
    → apex(feet) ≈ 2.79u (Physics2D 시뮬 측정값). 이 범위 안에서 층간 top-to-top 1.9~2.05u.
  · 좌측 타워 = 우상향 계단(폭 6/4/3), 우측 타워 = 지그재그(폭 5/3.5/2.5) — 단조롭지 않게
    구성·길이 다양화(INBOX 3). 좌표는 위 "레벨 레이아웃" 절.
  · 각 상단 플랫폼 x 범위를 하단과 1.5u+ 겹치게 둬서 직상 점프로 안착. 층간 간격을 캐릭터
    높이(1.6)보다 넉넉히 벌려 원웨이 모서리 걸림 방지(간격 1.6 이면 실패하는 것 시뮬로 확인).
- 확인한 것: EditMode 65/65, PlayMode 65/65(무변화). 콘솔 에러 0(TestResults 저장 로그만).
  Physics2D.simulationMode=Script 수동 시뮬레이션(에디터 비포커스라 FixedUpdate 미실행 →
  rb 속도 직접 주입, 메모리 참고):
  · 점프 상승 6종: 지면→L1, L1→L2, L2→L3, 지면→R1, R1→R2, R2→R3 — 전부 목표 플랫폼 top+0.8
    ±0.05 로 안착.
  · 드롭다운 6종: L3→L2, L2→L1, L1→지면, R3→R2, R2→R1, R1→지면 — IgnoreCollision 0.3s 창을
    재현해 전부 아래층에 안착.
  스크린샷 `Assets/Screenshots/level_inbox1-3.png` 직접 확인 — 녹색 지면 위 플레이어(중앙),
  좌측에 폭이 줄어드는 시안 3단 계단, 우측에 지그재그 시안 3단. 일반 회색 발판 사라짐.
- 커밋: 5e602a5 (Game.unity — 레벨 재설계). STATUS 갱신은 별도 커밋.
  ProjectSettings/Physics2DSettings.asset 의 serializedVersion 상승은 Unity 재직렬화
  부수효과(m_SimulationMode 0 유지)라 커밋 제외.
- 다음 할 일: 슬라이스 B (INBOX 2) — 스폰 지점을 6개 플랫폼 + 지면 위로 재배치,
  Inspector 리스폰 딜레이 확인, 새 레이아웃에서 FSM 다층 이동(점프/드롭다운) 검증.
